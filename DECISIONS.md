...
| V-10 | Does OGRE 14.5.2 + Vulkan actually **run** on Android? | Spike A on emulator (Android 16, x86_64, Vulkan 1.3) | ☑ **runs — content unverified, 2026-07-31** | **Six independent runs, consistent.** OGRE registers exactly one renderer ("Vulkan Rendering Subsystem"), initialises it, selects a real **Vulkan 1.4.0** device, creates a **2400×1080 surface and swapchain from the `ANativeWindow`**, and presents frames continuously at a **vsync-locked 60 fps** with an empty error log and a live, focused process. **The render path functions — this is the core Spike A result and it is a pass.** ❓ **Frame *content* is unverified**: every programmatic readback failed or lied (screencap uninformative; PNG/DDS/KTX codecs all unavailable or refusing; `copyContentsToMemory` returned `0 0 0 0` from **both** the swapchain **and** an offscreen target cleared to pure red, with the sentinel overwritten in each case). Two independent targets both returning exactly zero indicates **the readback zero-fills**, not that frames are black. ⚠️ **Reusable warning: do not use `copyContentsToMemory` to verify rendering on this backend — it yields confident wrong answers.** Resolution is a human looking at a screen, which the hardware pass does anyway. Emulator produces no Gate 1 evidence regardless (D-013). |
