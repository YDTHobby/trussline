...
- [ ] 🔴 `[HUMAN]` **Decide what happens to the in-game mod repository browser** (`GUI_RepositorySelector.cpp`). It calls **upstream's** repository API, which serves exactly the community content this fork legally cannot redistribute (LEGAL.md A4, R-21) — and it does so from *their* infrastructure. Leaving it wired up means a differently-branded product driving traffic to their servers to fetch content it can't ship. Options: disable it for now, repoint it at our own repository (Phase 8), or keep it as a user-side installer with clear provenance warnings. **Not a cosmetic call** — it sits on the legal and etiquette boundary at once.
...
**Exit criteria:**
- Fork builds for desktop unchanged, in CI, green.
- Android Studio + NDK + CMake toolchain installed and documented.
- Upstream OGRE-14/Cg status verified and recorded in DECISIONS.md.
- Device pool minimum (one midrange + one flagship) in hand.
...
- [ ] `[AI]` **Step 1 — fix the aliasing UB on x86 first (R-16).** Rewrite `ApproxMath.h:101-132` (`fast_invSqrt`, `approx_sqrt`, `approx_invSqrt`, `approx_exp`, `approx_pow2`, `approx_pow`) to use `memcpy` instead of `*(float*)&i` punning. Note the project builds at `-std=c++11`, so `std::bit_cast` is unavailable. Verify zero behavior change on desktop before touching ARM.
- [ ] `[AI]` **Step 2 — fix the `mirand` race.** `ApproxMath.h:28` is a header-scoped mutable `static int` mutated by `frand_11()` (`ActorForcesEuler.cpp:1671`) from inside the parallelized per-actor task — every translation unit gets its own copy and there's no synchronization. Make it `unsigned` (the `*= 16807` at `:35` also overflows a signed int, which is UB) and thread-local or per-actor. Multi-actor comparisons are untrustworthy until this is done.
...
- [ ] `[HUMAN]` **Step 7 — benchmark on the entry-tier device.** CPU % of one big core to sustain 2 kHz for a single vehicle; scaling across 1/2/4 actors; big vs LITTLE core placement. **Measure single-core headroom specifically (R-18):** the pool parallelizes *per actor*, never per node or beam (`ActorManager.cpp:1250-1294`), so one vehicle is bounded by one core's speed — the entry tier's weakest dimension.
