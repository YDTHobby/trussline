...
- ✅ **DEVICE IDENTIFIED 2026-07-31 — and reachable over wireless ADB, no USB to this machine required.**
  - **Xiaomi 23106RN0DA** (Redmi 13C class) · **MediaTek MT6768** (Helio G85 tier) · **Mali** GPU · Android **13** (API 33) · **arm64-v8a** · **Vulkan 1.1** (`0x401000`)
  - This is a genuinely entry-tier target and **harder than the emulator in every way that matters**: the emulator reported Vulkan **1.4** on an NVIDIA passthrough, this is Vulkan **1.1** on a real Mali driver — the exact family RISKS.md R-02 flags as the top "works on my phone" hazard. Android 13/API 33 comfortably clears our minSdk 29, and Vulkan 1.1 clears the manifest's `0x400003` requirement.
  - **Wireless ADB does not compromise benchmark validity**: ADB is only the deploy/log channel, so framerate, thermal behaviour, big.LITTLE scheduling and driver behaviour are all genuine. Avoid streaming verbose logcat during timed runs.
- **Superseded — emulation-first was the interim posture** while the phone was unavailable; it remains useful for fast iteration, but Gate 1 evidence now comes from this device. Installed images: `android-33/35/36/36.1` (x86_64); AVD `Pixel_7_Auto`. The superbuild accepts `TL_ANDROID_ABI=x86_64` with a loud non-shippable warning rather than failing.
...
| V-14 | Can the node/beam solver sustain **2 kHz** on the entry-tier phone? | Spike B benchmark on Xiaomi 23106RN0DA | ✅ **YES — with 4–7× margin against the Gate 1 threshold, 2026-07-31** | Measured on the real device (Helio G85: 6×1.7 GHz + 2×2.0 GHz), `-O2 -ffast-math` matching RoR's release flags. Budget is one core's **500 µs** real-time window per 2 kHz step; Gate 1 asks for one representative vehicle **under ~30%**. Results: **DAF semi (79n/315b, real content) 20.4 µs = 4.1%** · **Agora bus (151n/675b, real content) 21.4 µs = 4.3%** · typical-with-wheels (220n/1100b, est.) 34.7 µs = 6.9% · heavy (400n/2000b, est.) 64.0 µs = 12.8% · 4× DAF semi 45.0 µs = 9.0%. Single-threaded on one core, which is the correct test given R-18 (parallelism is per-actor, so one vehicle is bound to one core). ⚠️ **This is a LOWER BOUND, not the full solver.** It faithfully transcribes `CalcBeams` and `CalcNodes` on the real `node_t`/`beam_t` layout, but excludes wheels, shocks2/3, hydros, commands, ties, ropes, triggers, engine/powertrain, aerodynamics, buoyancy, replay, and real terrain collision (flat-plane stub). Those add cost. The margin is large enough that even a 3× underestimate keeps a typical vehicle near 21%, still inside the threshold — but Phase 4 must profile the real thing. Extracting the full solver means stubbing 21 includes and ~200 `App::` accesses; that is Phase-2-sized work, not spike-sized. |
| V-13 | Does it run on **real entry-tier hardware with a Mali driver**? | Xiaomi 23106RN0DA over wireless ADB | ✅ **YES — full pass, 2026-07-31** | **Every Spike A milestone passes on real hardware.** Xiaomi 23106RN0DA · MediaTek MT6768 (Helio G85 tier) · Android 13 · arm64-v8a. OGRE selects **Vulkan API 1.1.177**, **Vendor ID `0x13b5` = ARM (Mali)**, present mode `FIFO_KHR`. RenderWindow 1532×720 created from the `ANativeWindow`; **RTSS generated and validated a shader on the Mali driver**; lit triangle renders (centre pixel `255 179 113 255`); **~59.5 fps sustained** (120 frames per 2.016 s, vsync-locked); zero crashes, process alive throughout. Screenshot: `android/spike-a/spikea-device.png`. **Why this matters most:** RISKS.md R-02 names Mali as the top "works on my phone" hazard, and the emulator could never test it — it reported Vulkan 1.4 on an NVIDIA passthrough. This is Vulkan **1.1** on a genuine Mali driver, a materially harder target, and it passed **first try with no driver workarounds**. ⚠️ **Scope: one triangle.** This proves the render path, shader generation and driver compatibility. It says **nothing** about RoR's actual load — vehicle meshes, terrain, 2 kHz physics. Gate 1's framerate threshold still needs a representative scene. |
